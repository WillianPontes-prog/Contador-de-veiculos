...
GeradorDeTestes* criaGeradorDeTestes ()
{
	...
	/* Lê TODAS as imagens de fundo e de caminhos. */
	g->todos_backgrounds = (Imagem3C**) malloc (sizeof (Imagem3C*) * N_BACKGROUNDS);
	g->todos_paths = (Imagem1C**) malloc (sizeof (Imagem1C*) * N_BACKGROUNDS);
    for (i = 0; i < N_BACKGROUNDS; i++)
	{
		g->todos_backgrounds [i] = abreImagem3C (BACKGROUNDS [i]);
		if (!g->todos_backgrounds [i])
			return (NULL);
        g->todos_paths [i] = abreImagem1C (PATHS [i]);
		if (!g->todos_paths [i])
			return (NULL);
	}

	/* Por segurança, já inicia com o 1o background. */
	g->background = g->todos_backgrounds [0];
	g->path = g->todos_paths [0];
...
void destroiGeradorDeTestes (GeradorDeTestes* g)
{
	...
	/* Joga fora as imagens dos veículos, backgrounds e paths. */
	for (i = 0; i < N_VEICULOS; i++)
	...
	for (i = 0; i < N_BACKGROUNDS; i++)
    {
        destroiImagem3C (g->todos_backgrounds [i]);
        destroiImagem1C (g->todos_paths [i]);
    }
    free (g->todos_backgrounds);
    free (g->todos_paths);
...
Imagem3C* geraCasoDeTeste (GeradorDeTestes* g, int seed)
{
	...
	/* Sorteia um dos fundos / caminhos. */
	int bg = rand () % N_BACKGROUNDS;
	g->background = g->todos_backgrounds [bg];
	g->path = g->todos_paths [bg];
